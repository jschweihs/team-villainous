    ...
    <form @submit.prevent="save">
      ...
      <select v-model="event.game_id">
        <option value="0" disabled selected="selected"
          >Choose game (if applicable)</option
        >
      ...
      <div class="multi-select" v-for="index in numUsers" :key="index">
        ...
        <i
          ...
          @click="removeUserField(index - 1)"
        ></i>
      </div>
      <p class="form-action mb1" @click="addUserField">Add Another User</p>

      <label>Placements</label>
      <div class="multi-select" v-for="count in numPlacements" :key="count">
        <input
          type="text"
          class="multi-input"
        ...
        <i
          class="fas fa-trash-alt select-icon"
          @click="removePlacementField"
        ></i>
      </div>
      <p class="form-action" @click="addPlacementField">
  ...
  methods: {
    ...
    removePlacementField(i) {
      ...
      this.event.placements = this.event.placements
        .slice(0, i)
        .concat(
          this.event.placements.slice(i + 1, this.event.placements.length)
        );
    },
    addUserField() {
      this.numUsers++;
