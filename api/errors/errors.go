...
func New(status int, param string, detail string) *Error {
...
func Default(logger *log.Logger, w http.ResponseWriter, e *Error) {
	// Wrap the error in the top level errors array
	...
	// Handle HTML escape rule and indendation settings
...
// Params renders API paremeter errors
//
// This is a helper function used by the API endpoint handlers to make it
// easier to render parameter errors return from services
